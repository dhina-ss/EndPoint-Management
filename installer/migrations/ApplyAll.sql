...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20260730101846_AddWorkSessions') THEN
    ALTER TABLE devices ADD "SuspendedAt" timestamp with time zone;
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20260730101846_AddWorkSessions') THEN
    CREATE TABLE work_session_records (
        "Id" uuid NOT NULL,
        "DeviceId" uuid NOT NULL,
        "WorkDate" date NOT NULL,
        "WorkedSeconds" integer NOT NULL,
        "LastUpdated" timestamp with time zone NOT NULL,
        CONSTRAINT "PK_work_session_records" PRIMARY KEY ("Id"),
        CONSTRAINT "FK_work_session_records_devices_DeviceId" FOREIGN KEY ("DeviceId") REFERENCES devices ("Id") ON DELETE CASCADE
    );
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20260730101846_AddWorkSessions') THEN
    CREATE UNIQUE INDEX "IX_work_session_records_DeviceId_WorkDate" ON work_session_records ("DeviceId", "WorkDate");
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20260730101846_AddWorkSessions') THEN
    INSERT INTO "__EFMigrationsHistory" ("MigrationId", "ProductVersion")
    VALUES ('20260730101846_AddWorkSessions', '8.0.11');
    END IF;
END $EF$;
COMMIT;
